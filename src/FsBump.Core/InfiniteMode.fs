        ...
        for path in activePaths do
            ...
            if Vector3.Distance(playerPos, path.Position) < 80.0f then
                ...
                let rec retry attempt =
                    ...
                    // Turn logic
                    let nextState = 
                        { endState with CurrentColor = (path.CurrentColor + 1) % 4 }
                        |> StateOps.turn (
                    ...
                    // Overlap check
                    if not (Validation.checkOverlap (List.ofArray tiles) obstacles config) then
                        // Success
                        ...
                        if activePaths.Length < 3 && env.Random.NextDouble() < 0.1 then
                            let branchDir = 
                                if env.Random.NextDouble() < 0.5 then
                                    Vector3.Transform(nextState.Direction, Matrix.CreateRotationY(MathHelper.PiOver2))
                                else
                                    Vector3.Transform(nextState.Direction, Matrix.CreateRotationY(-MathHelper.PiOver2))
                            ...
                            graph <- PathGraphSystem.addBranch graph nextState.Id nextState.Position branchDirRounded
                        ...
                        newTiles.AddRange(t)
                        graph <- PathGraphSystem.updatePathState graph { s with PreviousDirection = path.Direction }

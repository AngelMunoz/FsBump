...
module StateOps =
  let advance dist (state: PathState) = {
    state with
        Position = VectorMath.add state.Position (VectorMath.mul state.Direction dist)
        DistanceFromStart = state.DistanceFromStart + dist
  }

  let moveY offset (state: PathState) = {
    state with
        Position = VectorMath.add state.Position (VectorMath.create 0.0f offset 0.0f)
  }
